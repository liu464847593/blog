...
- 箭头函数没有自己的`arguments`
- 箭头函数没有原型
- 箭头函数不用用作`Generator`函数，不能使用`yeild`关键字

### promise、async/await、Generator的区别
- `promise` 有三种状态，只能从 `pending -> fulfilled` 或者`pending ->rejected`
- `async/await` `Generator`函数的语法糖，基于`promise`看起来像同步代码,不需要链式调用 
- `Generator` 可以暂停和继续执行函数，用`next()`才能进行下一步
...
![](/asstes/img/InterviewQuestions/mvc.jpeg)
...
![](/asstes/img/InterviewQuestions/mvvm.jpeg)   
